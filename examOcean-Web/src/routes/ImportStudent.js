...
import styles from './ImportStudent.css'
...
const ImportStudent = ({
  ...
  const uploadProps = {
    ...
    onRemove: (file) => {
      ...
      dispatch({ type: 'ImportStudent/uploadFileList', payload: newFileList });
    },
    beforeUpload: (file) => {
  ...
  function handleOk () {
    validateFieldsAndScroll((errors, values) => {
      if (errors) {
        ...
        dispatch({ type: 'ImportStudent/ImportStudent', payload: formData });
      }
    })
  }
  ...
  if(isUploadSucces){
    ...
    notification["success"]({
      ...
      duration: 1,
      onClose:function(){
        dispatch(routerRedux.replace('/teacher/student'));
      }
    });
  }
    ...
    <div>
      ...
      <div className={styles.typeRow}>
        ...
        <FormItem
            ...
            colon={false}
            >
           <a href="/file/template/question" target="_blank" className={styles.templateDownload}><Icon type="file-excel"  /> 下载学生模版</a>
...
function mapStateToProps(state) {
  const fileList = state.ImportStudent.fileList;
  const isConnectionBad = state.ImportStudent.isConnectionBad;
  const isUploadSucces = state.ImportStudent.isUploadSucces;
  const tipMsg=state.ImportStudent.tipMsg;
  const isUploadError=state.ImportStudent.isUploadError;

  return {
    loading: state.loading.models.ImportStudent,
    fileList,
    isConnectionBad,
    isUploadSucces,
    tipMsg,
...
export default connect(mapStateToProps)(Form.create()(ImportStudent))
